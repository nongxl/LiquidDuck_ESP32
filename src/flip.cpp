...
float flip_boundary_energy = 0.0f;
...
static void handle_particle_collisions(int n, float* pos, float* vel,
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (x < min_x) { 
            x = min_x; 
            float e = fabsf(vel[2 * i + 0]); 
            if (e > flip_boundary_energy) flip_boundary_energy = e; 
            vel[2 * i + 0] = 0.0f; 
        }
        if (x > max_x) { 
            x = max_x; 
    ...
    for (int component = 0; component < 2; component++) {
        ...
        for (int i = 0; i < num_particles; i++) {
            ...
            if (to_grid) {
                ...
                float valid0 = (cell_type[nr0] != AIR_CELL || (nr0 >= offset && cell_type[nr0 - offset] != AIR_CELL)) ? 1.0f : 0.0f;
                float valid1 = (cell_type[nr1] != AIR_CELL || (nr1 >= offset && cell_type[nr1 - offset] != AIR_CELL)) ? 1.0f : 0.0f;
                float valid2 = (cell_type[nr2] != AIR_CELL || (nr2 >= offset && cell_type[nr2 - offset] != AIR_CELL)) ? 1.0f : 0.0f;
                float valid3 = (cell_type[nr3] != AIR_CELL || (nr3 >= offset && cell_type[nr3 - offset] != AIR_CELL)) ? 1.0f : 0.0f;
...
static void get_led_grid(const FlipFluid* f, float* out_grid, int visible_x, int visible_y) {
    const int padding = 1;
...
void flip_set_solver_quality(FlipFluid* f, int push_iters, int pressure_iters, float flip_ratio) {
    ...
    if (flip_ratio < 0.0f) flip_ratio = 0.0f;
    if (flip_ratio > 1.0f) flip_ratio = 1.0f;
...
void flip_step(FlipFluid* f, float dt, float gx, float gy) {
    ...
    transfer_velocities(1, f->flip_ratio, f->num_particles, f->particle_pos, f->particle_vel,
    ...
    transfer_velocities(0, f->flip_ratio, f->num_particles, f->particle_pos, f->particle_vel,
                        f->u, f->v, f->du, f->dv, f->prev_u, f->prev_v, f->cell_type, f->s,
                        f->f_num_x, f->f_num_y, f->h, f->f_inv_spacing,
                        f->AIR_CELL, f->FLUID_CELL, f->SOLID_CELL);
}
...
void flip_get_particles(const FlipFluid* f, int* out_num, float** out_pos, float** out_vel) {
    if (!f) return;
    if (out_num) *out_num = f->num_particles;
    if (out_pos) *out_pos = f->particle_pos;
    if (out_vel) *out_vel = f->particle_vel;
}
